...
def max_pool_1d(input, s):
	...
	:return: A tensor after average pooling
	'''
	return tf.nn.max_pool(input, [ 1, 1, s, 1 ], [1, 1, s, 1 ], 'VALID')
...
def batch_normalization_layer(inputs, is_training, decay=0.999, epsilon=1e-3):
	'''
	...
	# batch_mean, batch_var = tf.nn.moments(inputs,
	# 	range(len(inputs.get_shape())-1))
	# train_mean = tf.assign(pop_mean,
	# 	pop_mean * decay + batch_mean * (1 - decay))
	# train_var = tf.assign(pop_var,
	# 	pop_var * decay + batch_var * (1 - decay))
	# with tf.control_dependencies([train_mean, train_var]):
	# 	train =  tf.nn.batch_normalization(inputs,
	# 		batch_mean, batch_var, beta, scale, epsilon)

	# test = tf.nn.batch_normalization(inputs,
	# 	pop_mean, pop_var, beta, scale, epsilon)
	# return tf.cond(is_training, lambda: train, lambda: test)
...
def bn_relu_conv_dropout(current, in_features, out_features, kernel_size, is_training, stride=1, keep_prob=1):
	...
	:param is_training: Boolean, for the batch normalization
...
def residual_block(input_layer, output_channel, is_training, keep_prob=1 ,first_block=False, SE=False, ratio=16):
	...
	:param is_training: Boolean, for the batch normalization
	:param keep_prob: A scalar, for dropout
